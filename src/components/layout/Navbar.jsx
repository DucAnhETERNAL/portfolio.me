...
const Navbar = () => {
  ...
  // Update indicator position when language changes (text length changes)
  useEffect(() => {
    // Use requestAnimationFrame + setTimeout to ensure DOM has fully updated with new text
    ...
    // Small delay to ensure text has rendered
        ...
        <div className="flex items-center justify-between relative">
            ...
            >
              ...
              {NAV_LINKS.map((link, index) => (
                <button
                  ...
                  `}
                >
                  {t(`nav.${link.section === 'hero' ? 'home' : link.section}`)}
                </button>
              ))}
            </div>
          </div>

          {/* Theme Toggle & Language Switcher - Right Side */}
          <div className="hidden md:flex items-center gap-2 z-10">
            ...
            <div className="relative" ref={langMenuRef}>
              ...
              >
                <Globe className="w-5 h-5 text-gray-700 dark:text-dark-200" />
              </button>
              {isLangMenuOpen && (
                <div className="absolute right-0 mt-2 w-32 bg-white dark:bg-dark-800 rounded-lg shadow-xl border border-gray-200 dark:border-dark-700 overflow-hidden z-50">
                  <button
                    onClick={() => {
                      changeLanguage('en');
                      setIsLangMenuOpen(false);
                      // Update indicator position after language change
                      setTimeout(() => {
                        updateIndicatorPosition(activeSection);
                      }, 100);
                    }}
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-2 text-left text-sm transition-colors ${language === 'vi'
                      ? 'bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400'
                      : 'text-gray-700 dark:text-dark-200 hover:bg-gray-100 dark:hover:bg-dark-700'
                      }`}
                  >
